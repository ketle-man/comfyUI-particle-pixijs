...
PixiJS v7 をベースに、煙・火花・光線・スターワープの4種類のパーティクルと13種類のポストエフェクトフィルター、ブレンドモード切り替えを搭載しています。英字・数字をパーティクルシェイプとして使用する機能やキャンバス全面散布モードも備えています。
...
- **文字シェイプ** — アルファベット・数字をパーティクルシェイプとして指定（大文字・小文字可）
...
フィルターライブラリの **Particle** パネル下部でアルファベット・数字をパーティクルの形状として指定できます。

- **テキストボックス**: カンマ区切りで英数字を入力（例: `A,B,C,1,2,3`）。大文字・小文字どちらも指定可
- **[A-Z]** ボタン: アルファベット大文字 A〜Z を一括入力
- **[0-9]** ボタン: 数字 0〜9 を一括入力
- **[Clear]** ボタン: 文字シェイプ設定をクリア

文字シェイプが設定されている場合、カスタムテクスチャよりも低い優先度で適用され、プリセットシェイプより高い優先度になります。優先順は `カスタムテクスチャ > 文字シェイプ > プリセットシェイプ` です。
